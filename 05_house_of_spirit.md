...
Frees a fake fastbin chunk to get malloc to return a nearly-arbitrary pointer.
malloc이 임의에 가까운 포인터를 반환하도록 가짜 fastbin 청크를 해제하는 것이다.
...
This region (memory of length: 80) contains two chunks. The first starts at 0x7fffd6d63538 and the second at 0x7fffd6d63568.
이 공간은 (메모리의 길이 : 80) 두 개의 청크를 가지고 있다. 첫 번째는 0x7fffd6d63538 에서 시작하고, 두 번째는 0x7fffd6d63568 에서 시작한다.
...
fastbin 카테고리로 분별되는 동안 (x64 시스템에서는 128 이하) 이 청크의 chunk.size 값은 이 청크의 크기보다 16 더 큰 값이다. (청크 데이터를 수용하기 위해서)
...
PREV_INUSE 비트는 fastbin 에 속하는 크기인 청크에서 free에 의해 무시되며, IS_MMAPPED 와 NON_MAIN_ARENA 비트가 문제를 부른다.
...
... 노트 : 이 것은 malloc 의 구현에 의해 내부 값으로 올림 되어 다음 malloc 요청이 있을 때 반환 값을 결정하는데 사용되는 크기가 된다.

E.g. on x64, 0x30-0x38 will all be rounded to 0x40, so they would work for the malloc parameter at the end.
예시로 x64 시스템에서는, 0x30 에서 0x38 은 모두 0x40 으로 올림되며, 그것들은 마지막에 malloc 의 매개 변수로 사용될 것이다.

The chunk.size of the *next* fake region has to be sane.
가짜 구역의 chunk.size 값은 이상이 없어야 한다.

That is > 2*SIZE_SZ (> 16 on x64) && < av->system_mem (< 128kb by default for the main arena) to pass the nextsize integrity checks.
이 값이 다음 크기의 무결성 검사를 통과히기 위해서는 2 * SIZE_SZ (x64 시스템에서는 16) 값보다 커야하고, av->system_mem (main arena의 기본은 128kb)보다 작아야 한다.

No need for fastbin size.
fastbin 의 크기는 필요 없다.

Now we will overwrite our pointer with the address of the fake region inside the fake first chunk, 0x7fffd6d63538.
이제 우리의 포인터에 가짜 구역의 가짜 첫 번째 청크의 주소인 0x7fffd6d63538 값을 넣는다.

... note that the memory address of the *region* associated with this chunk must be 16-byte aligned.
... 노트 : 구역과 관련된 주소들은 16 바이트 단위로 정렬되어 있어야 한다.

Freeing the overwritten pointer.
덮어쓴 포인터를 할당 해제한다.

Now the next malloc will return the region of our fake chunk at 0x7fffd6d63538, which will be 0x7fffd6d63540!
이제 다음 malloc 는 0x7fffd6d63538 에 있는 가짜 청크의 0x7fffd6d63540 구역을 리턴해 준다.
...
[Here (hack.lu CTF 2014-OREO)](https://github.com/ctfs/write-ups-2014/tree/master/hack-lu-ctf-2014/oreo)